...
\begin{enumerate}
 \item Strategy -        was used extensively in the backend implementation, especially in the DBInterface class, which is responsible for passing queries to the database. 
                         \verb|query, insert, update| and \verb|delete| methods take as a parameter an interface, which represents an appropriate SQL query (\verb|SELECT| 
                         query, \verb|INSERT| query, \verb|UPDATE| query, \verb|DELETE| query), and hence allows the methods to be reused for different queries, provided 
                         that they implement appropriate interface.
 \item Template Method - 
 \item Observer -        this pattern was utilised in order to implement the dynamic update of event (and calendar) data, i.e. prevent the the user from having to refresh the
                         page every time a data about one of the events or calendars (to which the user subscribed) changed.  
 \item Singleton - 
 \item Builder -
 \item Decorator -       in order to create a thread-safe version of HashSet, a wrapper class ConcurrentHashSet was implemented, which uses internally a read-write lock to
                         ensure thread-safety while accessing and modifying the set.
 \item Facade - 
 \begin{comment}
  - callback - theoretically not a pattern 
  - caching  - desing pattern???
 \end{comment}
\end{enumerate}
